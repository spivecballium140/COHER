...
class GoalAdjust:
    def __init__(self):
        ...
        self.opt = torch.optim.AdamW(self.my_model.parameters(), lr=0.001)
    ...
    def predict_delta(self, x):  # need padding
        with torch.no_grad():
            ...
            y = self.my_model(x_tensor)
            self.buffer.append(x + [-1])
            # print(x, y)
            if abs(y.item() - sum(x)/len(x)) > 0.3:
                return sum(x)/len(x) - 0.1
            else:
                if y.item() > sum(x)/len(x):
                    return sum(x)/len(x) - 0.1
                else:
                    return y.item()
    ...
    def train_goal_net(self):
        if len(self.buffer) > 20:  # 250 episodes after
            ...
            train_data = torch.tensor(labeled_data, dtype=torch.float32)
...
class Forread:
    def __init__(self, CE_Tnum, M_Jnum, M_OPTnum, Enum, Dnum, Cnum):
        ...
        self.Device_Cost = []        # 成本系数
        self.Device_Reliability = [] # 可靠性
        self.Device_Quality = []     # 质量
        self.Device_Security = []    # 安全性


# 读取参数文件
# 计算任务数量，制造任务数量，制造任务的子任务数量，边缘设备数量，设备数量，云服务器数量
def read_paras_txt(road_path, CE_Tnum, M_Jnum, M_OPTnum, Enum, Dnum, Cnum):
    ...
    for i in range(0, Enum, 1):
        for j in range(0, Dnum, 1):
            self.EtoD_Distance[i][j] = fs.pop(0)  #边缘服务器到设备的距离，在100-5000

    for i in range(0, Dnum, 1):
        for j in range(0, Dnum, 1):
            self.DtoD_Distance[i][j] = fs.pop(0) #设备到设备的距离/延迟
    ...
    #新增
    # Try to read new attributes, otherwise generate random values
    if len(fs) >= Dnum * 4:
        for _ in range(Dnum):
            self.Device_Cost.append(fs.pop(0))
        for _ in range(Dnum):
            self.Device_Reliability.append(fs.pop(0))
        for _ in range(Dnum):
            self.Device_Quality.append(fs.pop(0))
        for _ in range(Dnum):
            self.Device_Security.append(fs.pop(0))
    else:
        # Generate random values if not present in file
        print("Warning: New attributes not found in data file. Generating random values.")
        import random
        for _ in range(Dnum):
            self.Device_Cost.append(random.uniform(300, 3000))
        for _ in range(Dnum):  
            self.Device_Reliability.append(random.uniform(0.85, 0.99))
        for _ in range(Dnum):
            self.Device_Quality.append(random.uniform(0.85, 0.99))
        for _ in range(Dnum):
            self.Device_Security.append(random.uniform(0.85, 0.99))
