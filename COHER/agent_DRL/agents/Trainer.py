...
class Trainer(object):
    ...
    def create_agent_to_agent_group_dictionary(self):
        ...
        agent_to_agent_group_dictionary = {
            ...
            "TD3_HER": "Actor_Critic_Agents",
            "TD3_OTD": "Actor_Critic_Agents",
            "DDPG_OTD": "Actor_Critic_Agents",
            "DDPG_HER": "Actor_Critic_Agents",
            "TD3-OTD": "Actor_Critic_Agents",
            "DDPG-OTD": "Actor_Critic_Agents",
    ...
    def run_games_for_agents(self):
        ...
        for agent_number, agent_class in enumerate(self.agents):
            ...
            if hasattr(self.config, 'custom_load_path') and self.config.custom_load_path is not None:
                self.config.load_path = self.config.custom_load_path
            else:
                self.config.load_path = './my_data_and_graph/models/' + self.config.environment.env_name + '-' + agent_name + '-' + str(self.config.load_path_epix) + '.pt'
    ...
    def run_games_for_agent(self, agent_number, agent_class):
        ...
        for run in range(self.config.runs_per_agent):
            ...
            #支持HER的不会进行处理，不支持的会FlattenDictWrapper
