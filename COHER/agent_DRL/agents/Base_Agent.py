# =============================================================================
# Base Agent for Deep Reinforcement Learning
# =============================================================================
# This file is derived from the Base_Agent in:
# https://github.com/MISTCARRYYOU/objectiveHER4hybridjobscheduling
#
# Original work: Wang et al., "Hybrid Task Scheduling in Cloud Manufacturing 
# with Sparse-reward Deep Reinforcement Learning", IEEE TASE, 2022
# =============================================================================
...
class Base_Agent(object):
    ...
    def __init__(self, config):
        ...
        log_dir = os.path.join('logs', self.agent_name, config.environment.env_name)
        ...
        # Curriculum learning with threshold-based transition
        self.curriculum_stage = 0  # Current curriculum stage index
        self.obj_history = []  # History of Obj values for sliding window
        self.curriculum_triggered = False  # Whether curriculum has been triggered
    ...
    def apply_curriculum(self, episode_number):
        """Applies curriculum learning based on improvement threshold.
        
        Phase transitions are triggered when the relative improvement of the moving average 
        of the global objective Obj over consecutive windows of W episodes falls below threshold ϵ:
        |Obj_prev - Obj_curr| / |Obj_prev| < ϵ
        """
        if not hasattr(self.config, 'curriculum_schedule') or not self.config.curriculum_schedule:
            return
            
        schedule = self.config.curriculum_schedule
        
        # Get current Obj value from environment
        current_obj = self.environment.obtain_obj()
        self.obj_history.append(current_obj)
        
        # Get curriculum parameters
        window_size = getattr(self.config, 'curriculum_window_size', 50)  # W: sliding window size
        epsilon = getattr(self.config, 'curriculum_epsilon', 0.05)  # ϵ: improvement threshold
        min_episodes_per_stage = getattr(self.config, 'curriculum_min_episodes', 100)  # Minimum episodes per stage
        
        # Check if we have enough history to evaluate improvement
        if len(self.obj_history) < 2 * window_size:
            # Not enough data, stay in current stage
            target_objectives = schedule[self.curriculum_stage]["objectives"]
            self._set_objectives(target_objectives)
            return
        
        # Check if minimum episodes in current stage have passed
        if episode_number < min_episodes_per_stage * (self.curriculum_stage + 1):
            target_objectives = schedule[self.curriculum_stage]["objectives"]
            self._set_objectives(target_objectives)
            return
        
        # Calculate moving averages over consecutive windows
        prev_window = self.obj_history[-2*window_size:-window_size]
        curr_window = self.obj_history[-window_size:]
        
        obj_prev = np.mean(prev_window)
        obj_curr = np.mean(curr_window)
        
        # Calculate relative improvement
        if abs(obj_prev) < 1e-10:  # Avoid division by zero
            relative_improvement = 0.0 if abs(obj_curr) < 1e-10 else 1.0
        else:
            relative_improvement = abs(obj_prev - obj_curr) / abs(obj_prev)
        
        # Check if we should transition to next stage
        if relative_improvement < epsilon and self.curriculum_stage < len(schedule) - 1:
            self.curriculum_stage += 1
            self.logger.info(f"Curriculum Transition: Stage {self.curriculum_stage} at episode {episode_number}. "
                           f"Relative improvement: {relative_improvement:.6f} < {epsilon}")
            print(f"\033[92m\033[1m>>> Curriculum Transition: Entering Stage {self.curriculum_stage} "
                  f"({schedule[self.curriculum_stage]['complexity']}) at episode {episode_number}\033[0m\033[0m")
        
        # Apply current stage objectives
        target_objectives = schedule[self.curriculum_stage]["objectives"]
        self._set_objectives(target_objectives)
    
    def _set_objectives(self, target_objectives):
        """Helper method to set active objectives in environment"""
        current_active = getattr(self.environment, 'active_objectives_list', None)
        if current_active != target_objectives:
            self.environment.set_active_objectives(target_objectives)
            self.environment.active_objectives_list = target_objectives


    def reset_game(self):
        """Resets the game information so we are ready to play a new episode"""
        if hasattr(self.config, 'curriculum_schedule') and self.config.curriculum_schedule:
            self.apply_curriculum(self.episode_number)
    ...
    def run_n_episodes(self, num_episodes=None, show_whether_achieved_goal=True, save_and_print_results=True):
        ...
        if self.config.save_model: self.locally_save_policy(None)
